...
def make_Classification():
    if learning_Rate.get() and feature1.get() and feature2.get() and class1.get() and class2.get() and epochs.get() and MSE.get():
        accuracy = preprocess.fire(float(learning_Rate.get()), feature1.get(), feature2.get(), class1.get(), class2.get(),
                              int(epochs.get()),float(MSE.get()),
                              bias_btn.get())
...
# getting the MSE threshold
create_label(Window, "Enter MSE threshold:", 'Helvatical bold', 20, 6, 1, padx=20, pady=20)
MSE = ttk.Entry(Window, width=25)
MSE.grid(row=6, column=2, padx=0, pady=0)

# check the Bias
create_label(Window, "Bias:", 'Helvatical bold', 20, 7, 1, padx=20, pady=20)
bias_btn= IntVar()
ttk.Checkbutton(Window, text="Bias", variable=bias_btn, command=bias_btn_checked, onvalue=1,
                offvalue=0).grid(row=7, column=2)

# run classifier btn
ttk.Button(Window, text="Run Classifier", width=30, command=make_Classification) \
    .grid(row=8, column=1, padx=0, pady=20)


# accuracy value txt box
create_label(Window, "Accuracy :", 'Helvatical bold', 20, 9, 1, padx=20, pady=20)
...
accuracyEntry.grid(row=9, column=2, padx=0, pady=0)
